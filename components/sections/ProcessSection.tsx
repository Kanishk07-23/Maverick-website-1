  ...
  return (
    <section className="bg-[var(--background)] py-32 relative overflow-hidden" id="process" ref={containerRef}>
       ...
       <div className="max-w-4xl mx-auto px-6 relative">
          ...
          {/* Animated Glow Line drawing based on scroll */}
          <motion.div 
...
function ProcessStep({ step, index, isEven }: { step: typeof steps[0], index: number, isEven: boolean }) {
  ...
  // Custom hook to trigger animation when node reaches center screen
  const { scrollYProgress } = useScroll({
    target: ref,
  ...
  const colorFilter = useTransform(scrollYProgress, [0, 0.5], ['grayscale(100%)', 'grayscale(0%)']);
