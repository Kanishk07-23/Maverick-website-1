...
export function CardStack<T extends CardStackItem>({
  ...
  const onWheel = (e: React.WheelEvent) => {
    ...
    // Only trigger on clear horizontal scroll
    if (Math.abs(e.deltaX) > Math.abs(e.deltaY) && Math.abs(e.deltaX) > 15) {
      const now = Date.now();
      // Wait 800ms between transitions to allow trackpad inertia to settle
              ...
              return (
                <motion.div
                  ...
                  style={{
                    ...
                    border: isActive ? "1px solid rgba(147, 51, 234, 0.25)" : "1px solid rgba(255,255,255,0.4)",
                  }}
                  initial={
                    reduceMotion
                      ? false
...
function DefaultFanCard({ item, active }: { item: CardStackItem; active: boolean }) {
  return (
    <div
      className="relative h-full w-full overflow-hidden flex flex-col justify-end"
      style={{
    ...
    >
      {/* Subtle gradient highlight — only on active */}
      <div
        className="absolute inset-0 transition-opacity duration-500 pointer-events-none"
        style={{
          background: active
            ? "radial-gradient(ellipse at 30% 20%, rgba(147,51,234,0.08) 0%, rgba(37,99,235,0.06) 50%, transparent 100%)"
            : "transparent",
          opacity: active ? 1 : 0,
        }}
      />
      ...
      <div
        ...
        style={{
          boxShadow: active
            ? "inset 1px 1px 0 rgba(255,255,255,0.7), inset -1px -1px 0 rgba(255,255,255,0.2)"
      ...
      <div className="relative z-10 p-8">
        {item.tag && (
          <span className="inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest mb-3 text-purple-700 bg-purple-50 border border-purple-100">
            {item.tag}
          </span>
        )}

        <h3
          className="text-3xl md:text-4xl font-black tracking-tight mb-3 transition-all duration-300"
          style={{
            color: active ? "transparent" : "#111827",
            backgroundImage: active ? "linear-gradient(135deg, #9333ea, #2563eb)" : "none",
        ...
        {active && item.href && (
          <Link
            href={item.href}
            className="inline-flex items-center gap-2 mt-5 px-5 py-2.5 rounded-xl text-sm font-bold cursor-pointer transition-all duration-300 text-white"
            style={{
