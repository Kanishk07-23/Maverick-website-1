  ...
  (
    ...
    const measureAndCenterLists = (toIndex = index, animate = true) => {
      const centerTrack = (
        ...
        const targetY =
          contRect.height / 2 - rowH / 2 - toIndex * rowH;
    ...
    const cssVars: CSSProperties = {
      ...
      ["--fx-page-bg" as string]: colors.pageBg ?? "#0a0a0a",
      ["--fx-stage-bg" as string]: colors.stageBg ?? "#0a0a0a",
      ["--fx-gap" as string]: `${gap}rem`,
      ["--fx-grid-px" as string]: `${gridPaddingX}rem`,
      ["--fx-row-gap" as string]: "10px",
    };

    const fixedSectionHeight = `${Math.max(1, total + 1) * 100}vh`;
      ...
      >
        ...
        <div className="fx-scroll">
          <div
            ...
            ref={fixedSectionRef}
            style={{ height: fixedSectionHeight, position: "relative" }}
          >
        ...
        <style dangerouslySetInnerHTML={{ __html: `
          ...
          .fx-fixed {
            ...
            overflow: hidden; background: var(--fx-stage-bg);
          }
          .fx-grid {
            display: grid;
          ...
          .fx-header {
            ...
            font-size: clamp(1.8rem, 6vw, 6rem); line-height: 0.88;
          ...
          .fx-progress-fill {
            ...
            background: var(--fx-text); height: 100%; transition: width 0.3s ease;
          }
          .fx-progress-numbers {
            position: absolute; inset: auto 0 calc(100% + 8px) 0;
            display: flex; justify-content: space-between;
            font-size: 0.75rem; color: var(--fx-text); opacity: 0.7;
          }
          .fx-end { height: 100vh; display: grid; place-items: center; background: var(--fx-page-bg); }
          .fx-fin { transform: rotate(90deg); color: rgba(255,255,255,0.15); font-size: 0.75rem; letter-spacing: 0.2em; }
          @media (max-width: 768px) {
            .fx-content { grid-template-columns: 1fr; place-items: center; }
            .fx-left, .fx-right { display: none; }
            .fx-center { height: 100vh; }
            .fx-featured-title { font-size: clamp(2rem, 10vw, 3.5rem); }
          }
        `}} />
      </div>
    );
