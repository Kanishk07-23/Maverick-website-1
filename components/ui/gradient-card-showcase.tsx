...
export default function SkewCards({ cards: customCards }: { cards?: CardItem[] }) {
  ...
  return (
    <>
      <div className="flex justify-center items-center flex-wrap py-10 min-h-screen bg-gray-950">
        {cards.map(({ title, desc, gradientFrom, gradientTo, href }, idx) => (
          ...
          >
            ...
            {/* Content */}
            <div className="relative z-20 left-0 p-[20px_40px] bg-[rgba(255,255,255,0.05)] backdrop-blur-[10px] border border-white/10 shadow-lg rounded-lg text-white transition-all duration-500 group-hover:left-[-25px] group-hover:p-[60px_40px] h-full flex flex-col justify-between">
              <div>
                <h2 className="text-2xl font-bold mb-4 font-['Poppins']">{title}</h2>
                <p className="text-sm leading-relaxed text-gray-300 font-medium">{desc}</p>
              </div>
              <a
                href={href || "#"}
